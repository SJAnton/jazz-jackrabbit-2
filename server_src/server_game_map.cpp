...
// Implement your class for the server game map here

void ServerGameMap::addObject(int x, int y, const std::string &type) {
    ...
    if (layerIndex >= 0 && layerIndex < (int)layers.size()) {
        for (int y = 0; y < 13; ++y) {
            for (int x = 0; x < layers[layerIndex].width; ++x) {
                ...
                std::cout << "Hay un tile de  en la posición (" << x << ", " << y << ")" << std::endl;
            }
        }
        for (int x = 0; x < layers[layerIndex].width; ++x) {
            layers[layerIndex].setTile(x, 13, 0x01);
            std::cout << "Hay un tile en la posición (" << x << ", " << 13 << ")" << std::endl;
        }
    }
}
...
void ServerGameMap::send_map(Queue<QueueData>* sndr_q) {
