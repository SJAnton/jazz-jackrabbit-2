...
ServerAcceptor::ServerAcceptor(
    Socket &socket, GameMapReader &map_reader, bool &was_closed) : 
        sk(socket),  map_reader(map_reader), was_closed(was_closed) {}
    ...
    while (!was_closed) {
        try {
            ...
            Client *client = new Client(
                std::move(peer), id, gmlp_id, gameloops, map_reader
            );
            client->start();
