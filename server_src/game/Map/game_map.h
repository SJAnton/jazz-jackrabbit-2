...
class GameMap {
    public:
        ...
        GameMap(int numTiles, int numLayers, int layerWidth, int layerHeight,
        ...
        GameMap(int numTiles, uint8_t spawn_x, uint8_t spawn_y,
                      uint8_t layers_height, uint8_t layers_width,
        ...
        std::vector<std::shared_ptr<ObjectEnemy>> get_enemies();

        std::vector<std::shared_ptr<ObjectCollected>> get_objects();

        void addObject(std::shared_ptr<ObjectCollected> &object);
